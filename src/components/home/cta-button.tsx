...
type Cta = MarketConfig["primaryCta"];
...
/**
 * Every call to action on the homepage renders through this, so the look,
 * the analytics attribute and the external-link handling never drift apart.
 * `location` ends up in the GA event (see CtaTracker).
 */
export default function CtaButton({
  ...
  return (
    ...
    >
      {whatsapp ? <WhatsappGlyph /> : <CalendarDays className="w-[18px] h-[18px]" aria-hidden="true" />}
      {cta.label}
      {variant === "primary" && !whatsapp && <ArrowRight className="w-4 h-4" aria-hidden="true" />}
    </a>
  );
}
